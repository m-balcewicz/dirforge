...
assert_success "run_dirforge init journal --name 'Geophysics' --id 'GEO-2025-0451' --service" \
...
assert_success "run_dirforge init journal --name 'Nature Geoscience' --id 'NG-2024-12345' --service" \
...
assert_success "run_dirforge init journal --name 'IEEE Trans. Geoscience & Remote Sensing' --id 'TGRS-2025-0789' --service" \
...
assert_success "run_dirforge init journal --name 'Geophysics' --id 'GEO-2024-1234' --service" \
...
assert_success "run_dirforge init journal --name 'Test Journal' --id 'TEST-123' --service" \
  "Create first journal service project"

# Attempt to create the same project again should fail
assert_failure "run_dirforge init journal --name 'Test Journal' --id 'TEST-123' --service" \
...
# Test dry-run mode
output=$(run_dirforge --dry-run init journal --name 'Test Journal' --id 'DRY-RUN-123' --service 2>&1)
...
assert_failure "run_dirforge init journal --name '' --id 'TEST-123' --service" \
...
assert_failure "run_dirforge init journal --name 'Test Journal' --id '' --service" \
  "Empty manuscript ID should fail"

assert_failure "run_dirforge init journal --name 'Test Journal' --id 'INVALID@ID#' --service" \
  "Invalid characters in manuscript ID should fail"

assert_success "run_dirforge init journal --name 'Test Journal' --id 'VALID-ID_123.v2' --service" \
...
assert_failure "run_dirforge init journal --name 'Test Journal'" \
  "Missing --id flag should fail"

assert_failure "run_dirforge init journal --id 'TEST-123' --service" \
...
assert_failure "run_dirforge init journal --name 'Test Journal' --id 'TEST-123' --service --first" \
...
assert_success "run_dirforge init journal --name 'First Journal' --id 'FIRST-123' --service" \
...
assert_success "run_dirforge init journal --name 'Second Journal' --id 'SECOND-456' --service" \
  "Second journal service project should use existing JOURNAL_WORLD"
...
if echo "$help_output" | grep -qi "dirforge init journal --name.*--id"; then
...
assert_success "run_dirforge init journal --name 'Path Test Journal' --id 'PATH-TEST-123' --service" \
...
assert_success "run_dirforge init journal --name 'IEEE Trans. on Signal Processing' --id 'TSP-2025-01' --service" \
...
assert_success "run_dirforge init journal --name 'Nature/Science/Technology' --id 'NST-2025-01' --service" \
...
assert_success "run_dirforge init journal --name 'Constitution Journal' --id 'CONST-123' --service" \
...
error_output=$(run_dirforge init journal --name 'Test Journal' --id 'INVALID@ID!' --service 2>&1 || true)
...
error_output=$(run_dirforge init journal --name '' --id 'TEST-123' --service 2>&1 || true)
...
assert_success "run_dirforge init journal --name 'Journal A' --id 'A-001' --service" \
  "Create first project in Journal A"

assert_success "run_dirforge init journal --name 'Journal A' --id 'A-002' --service" \
  "Create second project in Journal A"

assert_success "run_dirforge init journal --name 'Journal B' --id 'B-001' --service" \
