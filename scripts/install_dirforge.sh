...
# Bootstrap existing workspace: create .integrity/ metadata at WORLD and project levels.
# Arguments: repo_root, lib_dir
bootstrap_workspace() {
    local repo_root="$1"
    local lib_dir="$2"

    # Resolve workspace root
    local workspace_root="${DIRFORGE_WORKSPACE_ROOT:-}"
    if [[ -z "$workspace_root" || ! -d "$workspace_root" ]]; then
        workspace_root="$HOME/Documents"
    fi

    if [[ ! -d "$workspace_root" ]]; then
        print_info "No workspace directory found at $workspace_root — skipping bootstrap."
        return 0
    fi

    # Find existing WORLDs
    local found_worlds=()
    for world_name in "${KNOWN_WORLDS[@]}"; do
        if [[ -d "${workspace_root}/${world_name}" ]]; then
            found_worlds+=("${workspace_root}/${world_name}")
        fi
    done

    if [[ ${#found_worlds[@]} -eq 0 ]]; then
        print_info "No existing WORLD directories found in $workspace_root — skipping bootstrap."
        return 0
    fi

    # Locate manifest
    local manifest_file=""
    if [[ -f "${lib_dir}/templates/template_versions.yaml" ]]; then
        manifest_file="${lib_dir}/templates/template_versions.yaml"
    elif [[ -f "${repo_root}/templates/template_versions.yaml" ]]; then
        manifest_file="${repo_root}/templates/template_versions.yaml"
    fi

    if [[ -z "$manifest_file" || ! -f "$manifest_file" ]]; then
        print_warning "No template_versions.yaml manifest found — skipping bootstrap."
        return 0
    fi
    ...
    if ! confirm "Bootstrap existing workspace?" "y"; then
        print_info "Skipping bootstrap. You can run it later with:"
        echo "  ./scripts/update_dirforge.sh --bootstrap"
        return 0
    fi
    echo
    ...
    for world_path in "${found_worlds[@]}"; do
        local world_type
...
# Auto-generated by install_dirforge.sh
...
EOF
            ...
            print_success "${world_type}: .integrity/world.yaml already exists"
        fi
...
EOF
            print_success "  ${project_name}: created .integrity/project.yaml"
            ((total_projects++))
        done
    done

    echo
    print_info "Bootstrap complete: ${total_worlds} WORLD(s), ${total_projects} project(s) initialized."
    return 0
}
...
run_wizard() {
    ...
    bootstrap_workspace "$repo_root" "$lib_dir"
...
main() {
    ...
    if [ "$wizard_mode" = true ]; then
        ...
        if perform_installation "$install_mode" "$repo_root"; then
            ...
            echo
            # Offer bootstrap for non-interactive mode too
            local lib_dir
            if [[ "$install_mode" == "system" ]]; then
                lib_dir="/usr/local/lib/dirforge"
            else
